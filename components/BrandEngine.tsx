...
import { PenTool, Zap, Sparkles, Copy, CheckCircle, Loader2 } from 'lucide-react';
...
const BrandEngine: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const post = await generateLinkedInPost(selectedSignal, tone);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1 overflow-hidden">
                ...
                <div className="bg-dark-card border border-dark-border rounded-xl p-6 flex flex-col">
                    {selectedSignal ? (
                        <>
                            <div className="mb-4">
                                <label className="text-xs text-slate-500 uppercase font-bold mb-2 block">Choose Tone</label>
                                <div className="flex gap-2 flex-wrap">
                                    {['Thought Leader', 'Controversial', 'Data-Driven', 'Celebratory'].map((t) => (
                                        <button 
                                            key={t}
                                            onClick={() => setTone(t as any)}
                                            className={`px-3 py-1.5 rounded-full text-xs font-bold border ${tone === t ? 'bg-white text-black border-white' : 'bg-slate-800 text-slate-400 border-slate-700'}`}
                                        >
                                            {t}
                                        </button>
                                    ))}
                            ...
                            <button 
                                ...
                                className="w-full bg-brand-600 hover:bg-brand-500 text-white py-3 rounded-lg font-bold flex justify-center items-center gap-2 mb-4"
                            >
                            ...
                            <div className="flex-1 bg-slate-900 border border-slate-800 rounded-lg p-4 font-mono text-sm text-slate-300 overflow-y-auto whitespace-pre-wrap leading-relaxed">
                                {generatedPost || "Select a signal and tone to draft your LinkedIn post..."}
                            </div>
                            
                            {generatedPost && (
                                <button 
                                    onClick={() => {navigator.clipboard.writeText(generatedPost); setCopied(true); setTimeout(() => setCopied(false), 2000)}}
                                    className="mt-4 flex items-center justify-center gap-2 text-brand-400 text-sm font-bold hover:text-white"
                                >
                                    {copied ? <CheckCircle size={16}/> : <Copy size={16}/>} Copy to Clipboard
                                </button>
                            )}
                        </>
                    ) : (
